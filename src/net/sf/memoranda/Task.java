 ...
 *
 */
/*$Id: Item.java,v 1.15 2005/12/01 08:12:26 alexeya Exp $*/
public class Task implements Item, Comparable {
     ...
     */
    public Task(Element ItemElement, ItemList tl) {
...
*/
	...
	/**
	 * A "Item rate" is an informal index of importance of the Item
...
/*	   Item t = (Item)Item;
	   switch (mode) {
		   case BY_IMP_RATE: return -1*calcItemRate(t, date);
...
*/
	...
	private Collection convertToItemObjects(Elements items) {
        ...
        for (int i = 0; i < items.size(); i++) {
            Item t = new Task(items.get(i), _tl);
            v.add(t);
        }
        return v;
    }
	 ...
	 */
	public Item getSubItems(String id) {
		Elements subItems = _element.getChildElements("Item");
		for (int i = 0; i < subItems.size(); i++) {
			if (subItems.get(i).getAttribute("id").getValue().equals(id))
				return new Task(subItems.get(i), _tl);
		}
		return null;
	}
